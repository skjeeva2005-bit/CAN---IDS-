...
$qq=mysqli_query($connect,"select * from cav_admin where bcode='$bc'");
...
$qq2=mysqli_query($connect,"select * from cav_data where user='$un'");
...
$sid=$rr2['satid'];
$qq3=mysqli_query($connect,"select * from cav_data where satid='$sid'");
...
////////////
$data="CAV Vehicle: $sid, Received Data: ".$loc;
$dtime=$rdate.", ".$rtime;
...
//////////
$mq=mysqli_query($connect,"select max(id) from cav_files");
$mr=mysqli_fetch_array($mq);
$id=$mr['max(id)']+1;
mysqli_query($connect,"insert into cav_files(id,satid,filename,user,encdata) values($id,'$sid','$loc','$un','$enc')");
...
<script>
							...
							setTimeout(function () {
							 ...
							 window.location.href="http://localhost/can/test3.php?bc=<?php echo $bc; ?>&act=<?php echo $act; ?>";
							}, 5000);
							</script> 
<?php
}
